...
BlockLibraryController.prototype.getCurrentBlockType = function() {
  ...
  var blockType = rootBlock.getFieldValue('DM_name').trim().toLowerCase();
...
BlockLibraryController.prototype.saveToBlockLibrary = function() {
  ...
  //Create preview block XML
  var xmlElementPreview = Blockly.utils.xml.createElement('xml');
  var blockPreview = BlockFactory.previewWorkspace.getTopBlocks()[0];
  xmlElementPreview.appendChild(Blockly.Xml.blockToDomWithXY(blockPreview));


  // Save block + add info to save to DB using textarea
  this.storage.addBlock(blockType, xmlElement);
  document.getElementById('rawXML').value = Blockly.Xml.domToPrettyText(xmlElement);
  //document.getElementById('rawXMLPreview').value = Blockly.Xml.domToPrettyText(xmlElementPreview);
  ...
  // Save block in database:
   document.getElementById('languageTA').value;

  this.storage.saveToLocalStorage();

    trick = true; //ugly trick
  ...
  //BlocklyDevTools.Analytics.onSave('Block');
   ...
   let DM = {};
   DM["id"] = document.getElementById('idDM').value;
   DM["name"] = document.getElementById('nameDM').value;
   DM["type"] = document.getElementById('typeDM').value; //quite confusing but here we save if the block is a report or a component
   DM["url"] = document.getElementById('urlDM').value;
